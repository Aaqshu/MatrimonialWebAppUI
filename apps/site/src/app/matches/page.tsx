...
export default function MatchesPage() {
  ...
  const renderCard = (item: InterestItem, isReceived: boolean) => {
    ...
    return (
      <div key={item.InterestRequestId} className="flex items-center gap-4 rounded-2xl border border-white/8 bg-white/[0.03] p-4 transition-colors hover:border-white/20">
        {photo ? (
          <img src={photo} alt={name} className="size-14 rounded-xl object-cover" />
        ) : (
          <div className="flex size-14 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500/25 to-amber-500/25 text-lg font-semibold">
            {initials}
          </div>
        )}
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <Link href={`/profile/${id}`} className="truncate font-medium hover:text-emerald-300">
              {name}{age ? `, ${age}` : ''}
            </Link>
            <span className={`rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide ${STATUS_STYLE[item.Status] || ''}`}>
              {item.Status}
            </span>
          </div>
          {city && <p className="mt-0.5 truncate text-xs text-white/40">{city}</p>}
        </div>
        {showActions && (
          <div className="flex shrink-0 gap-2">
            <button
              ...
              className="flex cursor-pointer items-center gap-1 rounded-full bg-emerald-500 px-3.5 py-1.5 text-xs font-medium text-emerald-950 transition-colors hover:bg-emerald-400 disabled:cursor-wait disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={busyId === item.InterestRequestId}
              className="flex cursor-pointer items-center gap-1 rounded-full border border-white/15 px-3.5 py-1.5 text-xs font-medium text-white/60 transition-colors hover:border-red-500/40 hover:text-red-300 disabled:cursor-wait disabled:opacity-50"
            >
        ...
        <div className="mt-4 space-y-3">
          {list.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-white/15 p-10 text-center">
              <CheckCheck className="mx-auto size-6 text-white/20" />
              <p className="mt-3 text-sm text-white/40">
                {tab === 'received' ? 'No interests received yet' : 'No interests sent yet'}
              </p>
              {tab === 'received' && (
                <Link href="/search" className="mt-2 inline-block text-sm text-emerald-400 hover:underline">
